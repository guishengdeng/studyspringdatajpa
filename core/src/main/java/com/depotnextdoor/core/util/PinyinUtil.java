...
public class PinyinUtil {
    ...
    public static Map<String, String> getPingYin(String src) {
        if (src != null) {
            ...
            String[] pinyinArray;
            Map<String, String> tempCharMap = new HashMap<>();
            Map<String, String> tempStrPinyinMap = new HashMap<>();
            Map<String, String> resultPinyinMap = new HashMap<>();
            String key_char, key_str;
            try {
        ...
        if (src != null) {
            ...
            try {
                for (int i = 0; i < charArray.length; i++) {
                    ...
                    // 判断是否为汉字字符函数
                    if (Character.toString(charArray[i]).matches("[\\u4E00-\\u9FA5]+")) {
                        pinyinArray = PinyinHelper.toHanyuPinyinStringArray(charArray[i], pinyinOutputFormat);
                        if (pinyinArray != null) {
                            for (int j = 0; j < pinyinArray.length; j++) {
                                tempCharMap.put(pinyinArray[j], Character.toString(charArray[i]));
                            }
