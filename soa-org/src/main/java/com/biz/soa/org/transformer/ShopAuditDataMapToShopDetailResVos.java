...
/**
 * 钻换集合
 * Created by dylan on 17-5-18.
 */
public class ShopAuditDataMapToShopDetailResVos implements Function<ShopAuditDataMap, List<ShopDetailResVo>>, Serializable {

    @Override
    public List<ShopDetailResVo> apply(ShopAuditDataMap input) {
        List<ShopDetailResVo> vos = null;
        if (input != null) {
            vos = newArrayList();
            Collection<ShopAuditVo> shopAuditVos =input.values();
            if(CollectionUtils.isNotEmpty(shopAuditVos)){
            for (ShopAuditVo shopAuditVo : shopAuditVos) {
                if (shopAuditVo != null) {
                    ShopDetailResVo vo = new ShopDetailResVo();
