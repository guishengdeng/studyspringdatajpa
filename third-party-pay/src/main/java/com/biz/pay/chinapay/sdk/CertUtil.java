 ...
 * =============================================================================
 */
package com.biz.pay.chinapay.sdk;
...
//	private static X509Certificate encryptTrackCert = null;
	...
	public static void init() {
		if (SDKConstants.TRUE_STRING.equals(SDKConfig.getInstance()
				.getSingleMode())) {
			// 单证书模式,初始化配置文件中的签名证书
			initSignCert();
		}
	...
	public static void initSignCert() {
		...
		try {
			keyStore = getKeyInfo(SDKConfig.getInstance().getSignCertPath(),
					SDKConfig.getInstance().getSignCertPwd(), SDKConfig
							.getInstance().getSignCertType());
	...
	private static void initEncryptCert() {
		LogUtil.writeLog("加载敏感信息加密证书==>"+SDKConfig.getInstance().getEncryptCertPath());
		if (StringUtils.isNotBlank(SDKConfig.getInstance().getEncryptCertPath())) {
			encryptCert = initCert(SDKConfig.getInstance().getEncryptCertPath());
...
//		}
	...
	private static void initTrackKey() {
		if (StringUtils.isNotBlank(SDKConfig.getInstance().getEncryptTrackKeyModulus())
				&& StringUtils.isNotBlank(SDKConfig.getInstance().getEncryptTrackKeyExponent())) {
			encryptTrackKey = SecureUtil.getPublicKey(SDKConfig.getInstance().getEncryptTrackKeyModulus(),
					SDKConfig.getInstance().getEncryptTrackKeyExponent());
	...
	private static void initValidateCertFromDir() {
		certMap.clear();
		String dir = SDKConfig.getInstance().getValidateCertDir();
	...
	public static PrivateKey getSignCertPrivateKey() {
		try {
			...
			PrivateKey privateKey = (PrivateKey) keyStore.getKey(keyAlias,
					SDKConfig.getInstance().getSignCertPwd().toCharArray());
			return privateKey;
	...
	public static PublicKey getEncryptCertPublicKey() {
		if (null == encryptCert) {
			String path = SDKConfig.getInstance().getEncryptCertPath();
